...
BOOL LaunchAppIntoDifferentSession(WCHAR* szExe, WCHAR* szDir, WCHAR* szLine)//???SYSTEM?????????????????????????
{
...
BOOL InstallService()//??????????
{
	...
	SC_HANDLE hService = ::CreateService(
		...
		SERVICE_AUTO_START, //??????ERVICE_DEMAND_START??????????????????????????
...
DWORD GetParentProcessID(DWORD dwProcessId)//?????????????ID
{
	if (dwProcessId == -1)
		return -1;
	LONG                        status;
...
BOOL EnableDebugPrivilege(BOOL bEnableDebugPrivilege)//DEBUG????
{
	HANDLE hToken;
	TOKEN_PRIVILEGES tp;
...
BOOL RunProcess(LPTSTR szExe, const WCHAR* szCommandLine,HANDLE *pProcess)/////////////////////////////////??????????
{
	...
	szLine[0] = L'\0';
	if(szCommandLine)
		lstrcpyn(szLine, szCommandLine, MAX_PATH);
	ret = CreateProcess(sz,// RUN_TEST.bat???????????????????
		szLine,
		NULL,
		NULL,
		FALSE,
...
*/
BOOL GetProcessFileName(DWORD dwProcessId, LPTSTR pszFileName, DWORD dwFileNameLength)////////////////////////????ID?????????????????????
{
	BOOL bResult = false;
...
/*
HICON GetUWPAppIcon(HWND hWnd, UINT uiIconSize = 32)////////////////////////////////////????UWP??????????
{
	HICON hIcon = NULL;
...
BOOL SetForeground(HWND hWnd)//????????????
{
	...
	return bResult;
	/*
		int tIdCur = GetWindowThreadProcessId(GetForegroundWindow(), NULL);//????????????????????????D
		int tIdCurProgram = GetWindowThreadProcessId(hWnd,NULL);//??????????????????????ID
...
BOOL OpenProcessPath(DWORD dwProcessId)//??????????ID?????????????
{
	BOOL ret = FALSE;
...
HICON OpenProcessIcon(DWORD dProcessID, int cx)//??????????ID????ICON
{
	HICON ret = NULL;
	WCHAR szExe[MAX_PATH];
