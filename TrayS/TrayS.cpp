...
DWORD GetPhysicalDriveFromPartitionLetter(WCHAR letter)//通过盘符获取物理硬盘位置
{
	...
	WCHAR path[64];
	swprintf_s(path, ARRAYSIZE(path), L"\\\\.\\%c:", letter);
			...
			if (PdhCloseQuery != NULL && PdhAddCounter != NULL && PdhCollectQueryData != NULL && PdhCloseQuery != NULL && PdhGetFormattedCounterValue != NULL && PdhRemoveCounter != NULL)
			{
				if (PdhOpenQuery(NULL, 0, &hQuery) == ERROR_SUCCESS)
				{
					...
					else
					{
						...
						swprintf_s(sz, ARRAYSIZE(sz), L"\\PhysicalDisk(%d %c:)\\Disk Read Bytes/sec", d, TraySave.szDisk);
						...
						swprintf_s(sz, ARRAYSIZE(sz), L"\\PhysicalDisk(%d %c:)\\Disk Write Bytes/sec", d, TraySave.szDisk);
						PdhAddCounter(hQuery, sz, NULL, &hDiskWrite);
						swprintf_s(sz, ARRAYSIZE(sz), L"\\PhysicalDisk(%d %c:)\\%% Idle Time", d, TraySave.szDisk);
...
//					SetParent(hTaskBar, GetForegroundWindow());
			...
			if(!TraySave.bMonitorFloat&&!bFullScreen && rovi.dwBuildNumber < 22000)
										...
										if (paa->IfIndex == mit2->Table[i].InterfaceIndex)
										{
											...
											while (pUnicast)
											{
												if (AF_INET == pUnicast->Address.lpSockaddr->sa_family)// IPV4 地址，使用 IPV4 转换
												{
													...
													byte* bp = (byte*)pAddr;
													swprintf_s(traffic[l].IP4, ARRAYSIZE(traffic[l].IP4), L"%d.%d.%d.%d", bp[0], bp[1], bp[2], bp[3]);
													break;
												}
...
void SetWH()
{
	...
	if (TraySave.bMonitorTraffic)
	{
		...
		{
			swprintf_s(sz, ARRAYSIZE(sz), L"M%s8.88", TraySave.szTrafficOut);
			::GetTextExtentPoint(mdc, sz, lstrlen(sz), &tSize);
		}
	...
	if (TraySave.bMonitorUsage)
	{
		...
		{
			swprintf_s(sz, ARRAYSIZE(sz), L"%s88%s", TraySave.szUsageMEM, TraySave.szUsageMEMUnit);
			::GetTextExtentPoint(mdc, sz, lstrlen(sz), &tSize);
		}
	...
	if (TraySave.bMonitorTemperature)
	{
		...
		{
			swprintf_s(sz, ARRAYSIZE(sz), L"%s88%s", TraySave.szTemperatureGPU, TraySave.szTemperatureGPUUnit);
			::GetTextExtentPoint(mdc, sz, lstrlen(sz), &tSize);
		}
	...
	if (TraySave.bMonitorDisk)
	{
		...
		{
			swprintf_s(sz, ARRAYSIZE(sz), L" M%s8.88", TraySave.szDiskReadSec);
			::GetTextExtentPoint(mdc, sz, lstrlen(sz), &tSize);
		}
...
void GetTrafficStr(WCHAR* sz, size_t bufSize, ULONG64 uByte, BOOL bBit, int iUnit)
{
	if (bBit)
		uByte *= 8;
	if (((uByte < 1024 && !bBit) || (uByte < 1000 && bBit)) && iUnit==0)
		swprintf_s(sz, bufSize, L"%dB", uByte);
	else if (((uByte < 1048576 && !bBit) || (uByte < 1000000 && bBit)) && iUnit != 2)
	{
		...
		if (k_byte >= 10000)
			swprintf_s(sz, bufSize, L"%dK", k_byte / 100);
		else if (k_byte >= 1000)
		{
			k_byte /= 10;
			swprintf_s(sz, bufSize, L"%d.%dK", k_byte / 10, k_byte % 10);
		}
		else
		{
			swprintf_s(sz, bufSize, L"%d.%dK", k_byte / 100, k_byte % 100);
		}
	}
	else if ((uByte < 1073741824 && !bBit) || (uByte < 1000000000 && bBit))
	{
		...
		if (m_byte >= 10000)
			swprintf_s(sz, bufSize, L"%dM", m_byte / 100);
		else if (m_byte >= 1000)
		{
			m_byte /= 10;
			swprintf_s(sz, bufSize, L"%d.%dM", m_byte / 10, m_byte % 10);
		}
		else
			swprintf_s(sz, bufSize, L"%d.%dM", m_byte / 100, m_byte % 100);
	}
	else if ((uByte < 1099511627776 && !bBit) || (uByte < 1000000000000 && bBit))
	{
		...
		if (g_byte >= 10000)
			swprintf_s(sz, bufSize, L"%dG", g_byte / 100);
		else if (g_byte >= 10)
		{
			g_byte /= 10;
			swprintf_s(sz, bufSize, L"%d.%dG", g_byte / 10, g_byte % 10);
		}
		else
			swprintf_s(sz, bufSize, L"%d.%dG", g_byte / 100, g_byte % 100);
	}
	else
	{
		ULONG64 t_byte;
		...
		if (t_byte >= 10000)
			swprintf_s(sz, bufSize, L"%dT", t_byte / 100);
		else if (t_byte >= 10)
		{
			t_byte /= 10;
			swprintf_s(sz, bufSize, L"%d.%dT", t_byte / 10, t_byte % 10);
		}
		else
			swprintf_s(sz, bufSize, L"%d.%dT", t_byte / 100, t_byte % 100);
	}
	if (bBit)
				...
				if (pt.y < (nTraffic + 13) * wTipsHeight)
				{
					...
					if (dwLen != 0)
					{
						...
						if (x < driver_number)
						{
							WCHAR sz[24];
							memset(sz, 0, 48);
							swprintf_s(sz, ARRAYSIZE(sz), L"o%s", &wDrive[x * 4]);
							RunProcess(NULL, sz);
						}
					}
				}
...
DrawText(mdc, traffic[i].szFriendlyName, lstrlen(traffic[i].szFriendlyName), &rc, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
					...
					*/					
					...
					GetTrafficStr(sz, ARRAYSIZE(sz), traffic[i].in_bytes, HIWORD(TraySave.iUnit));
					DrawText(mdc, sz, lstrlen(sz), &rc, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
					GetTrafficStr(sz, ARRAYSIZE(sz), traffic[i].in_byte, HIWORD(TraySave.iUnit));
					...
					GetTrafficStr(sz, ARRAYSIZE(sz), traffic[i].out_bytes, HIWORD(TraySave.iUnit));
					DrawText(mdc, sz, lstrlen(sz), &rc, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
					GetTrafficStr(sz, ARRAYSIZE(sz), traffic[i].out_byte, HIWORD(TraySave.iUnit));
				...
				for (int i = 0; i < 6; i++)
				{
					...
					*/
					...
					swprintf_s(sz, ARRAYSIZE(sz), L"%d.%.2d%%", iCpuUsage / 100, iCpuUsage % 100);
					DrawText(mdc, sz, lstrlen(sz), &rc, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
					RECT cr = rc;
					cr.left = crc.right * 100 / 145;
					cr.right = crc.right * 100 / 122;
					swprintf_s(sz, ARRAYSIZE(sz), L"%d", ppcu[i]->dwProcessID);
				...
				for (int i = 0; i < 6; i++)
				{
					...
					*/
					if (ppmu[i]->dwMemUsage >= 1048576000)
					{
						...
						swprintf_s(sz, ARRAYSIZE(sz), L"%d.%.2dGB", iMemUsage / 100, iMemUsage % 100);
					}
					else
					{
						SIZE_T iMemUsage = (ppmu[i]->dwMemUsage * 100 / 1048576);
						swprintf_s(sz, ARRAYSIZE(sz), L"%d.%.2dMB", iMemUsage / 100, iMemUsage % 100);
					}
					DrawText(mdc, sz, lstrlen(sz), &rc, DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
					RECT cr = rc;
					cr.left = crc.right * 100 / 145;
					cr.right = crc.right * 100 / 122;
					swprintf_s(sz, ARRAYSIZE(sz), L"%d", ppmu[i]->dwProcessID);
					DrawText(mdc, sz, lstrlen(sz), &cr, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
					cr.left = crc.right * 100 / 156;
					cr.right = crc.right * 100 / 145;
					if (PtInRect(&cr, pt))
				...
				/*
							...
							if (pCallNtPowerInformation(ProcessorInformation, NULL, 0, &pi[0], sizeof PROCESSOR_POWER_INFORMATION * dNumProcessor) == 0)
							{
								...
								swprintf_s(sz, ARRAYSIZE(sz),  L"%d个逻辑处理器 当前频率%d.%.2dGHz 最大频率%d.%.2dGHz", dNumProcessor, iCGhz/100,iCGhz%100, iMGhz / 100, iMGhz % 100);
							}
				...
				*/
				...
				swprintf_s(sz, ARRAYSIZE(sz), L"虚拟内存:%d.%.2d/%d.%.2dGB", iaPage / 100, iaPage % 100, itPage / 100, itPage % 100);
				...
				DeleteObject(hb3);
				DeleteObject(hb4);
				swprintf_s(sz, ARRAYSIZE(sz), L"物理内存:%d.%.2d/%d.%.2dGB", ia / 100, ia % 100, it / 100, it % 100);
				DrawText(mdc, sz, lstrlen(sz), &rc, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
...
void DrawDisk(HDC mdc, LPRECT lpRect, double dwByte,BOOL bReadWrite)
{
	...
	{		
		...
		if (f_byte >= 100)
			swprintf_s(sz, ARRAYSIZE(sz), L"%dM",  m_byte / 100);
		else if (f_byte >= 10)
			swprintf_s(sz, ARRAYSIZE(sz), L"%d.%.1dM", m_byte / 100, (m_byte / 10) % 10);
		else
			swprintf_s(sz, ARRAYSIZE(sz), L"%d.%.2dM", m_byte / 100, m_byte % 100);
	}
	else
	{
		f_byte /= 1073741824;
		...
		if (f_byte >= 100)
			swprintf_s(sz, ARRAYSIZE(sz), L"%dG", g_byte / 100);
		else if (f_byte >= 10)
			swprintf_s(sz, ARRAYSIZE(sz), L"%d.%.1dG", g_byte / 100, (g_byte / 10) % 10);
		else
			swprintf_s(sz, ARRAYSIZE(sz), L"%d.%.2dG", g_byte / 100, g_byte % 100);
	}
...
void DrawTraffic(HDC mdc, LPRECT lpRect, ULONG64 dwByte, BOOL bInOut)
{
	...
	if (dwByte < TraySave.dNumValues[0])
		...
		rgb = TraySave.cMonitorColor[3];
	SetTextColor(mdc, rgb);
	GetTrafficStr(sz, ARRAYSIZE(sz), dwByte, HIWORD(TraySave.iUnit),LOWORD(TraySave.iUnit));
	DrawShadowText(mdc, szT, lstrlen(szT), lpRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE, bColor, bShadow);
	DrawShadowText(mdc, sz, lstrlen(sz), lpRect, DT_RIGHT | DT_VCENTER | DT_SINGLELINE, bColor, bShadow);
}
...
INT_PTR CALLBACK TimeProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)//任务栏信息窗口过程
{
	...
	case WM_ERASEBKGND:
	{
		...
		if (mdc)
		{
			...
			{
				fsize = DPI(-11);
				swprintf_s(sz, ARRAYSIZE(sz), L"%.2d'%s", systm.wSecond, szWeek[systm.wDayOfWeek]);
			}
			else
			{
				fsize = DPI(-12);
				swprintf_s(sz, ARRAYSIZE(sz), L"%s%.2d:%.2d:%.2d", szWeek[systm.wDayOfWeek], systm.wHour, systm.wMinute, systm.wSecond);
			}
...
//				InflateRect(&rc, -1, -1);
				...
				if (TraySave.bMonitorUsage)
				{
					...
					*/
					if (TraySave.iMonitorSimple == 1)
						swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%%", iCPU);
					else if (TraySave.iMonitorSimple == 2)
						swprintf_s(sz, ARRAYSIZE(sz), L"%.2d", iCPU);
					else
						swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s",  iCPU, TraySave.szUsageCPUUnit);
					...
					*/
					if (TraySave.iMonitorSimple == 1)
						swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%%", MemoryStatusEx.dwMemoryLoad);
					else if (TraySave.iMonitorSimple == 2)
						swprintf_s(sz, ARRAYSIZE(sz), L"%.2d", MemoryStatusEx.dwMemoryLoad);
					else
						swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s", MemoryStatusEx.dwMemoryLoad, TraySave.szUsageMEMUnit);
					sLen = lstrlen(sz);
				...
				if (TraySave.bMonitorTemperature)
				{
					...
					if (bPawnIoReady)
					{
						...
						if ((hATIDLL != NULL || hNVDLL != NULL )&& TrayData->iTemperature1 == TrayData->disktime && TraySave.bMonitorDisk&&!hOHMA)
						{
							if (TraySave.iMonitorSimple == 0)
							{
								...
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s", TrayData->iTemperature1, TraySave.szUsageMEMUnit);
							}
							else if (TraySave.iMonitorSimple == 1)
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%%", TrayData->iTemperature1);
							else
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d", TrayData->iTemperature1);
						}
						else
						{
							...
							else
							{
								...
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s", TrayData->iTemperature1, TraySave.szTemperatureCPUUnit);
							}
						}
					...
					if (hATIDLL == NULL && hNVDLL == NULL && TraySave.bMonitorDisk&&!hOHMA)
					{
						...
						{
							DrawShadowText(mdc, TraySave.szDiskName, lstrlen(TraySave.szDiskName), &crc, DT_LEFT | DT_VCENTER | DT_SINGLELINE, bColor, bShadow);
							swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s" , TrayData->iTemperature2, TraySave.szUsageMEMUnit);
						}
						else if (TraySave.iMonitorSimple == 1)
							swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%%", TrayData->iTemperature2);
						else
							swprintf_s(sz, ARRAYSIZE(sz), L"%.2d", TrayData->iTemperature2);
					}
					else
					{
						if (TraySave.iMonitorSimple == 0)
						{
							...
							swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s", TrayData->iTemperature2, TraySave.szTemperatureGPUUnit);
						}
						else if (TraySave.iMonitorSimple == 1)
							swprintf_s(sz, ARRAYSIZE(sz), L"%.2d℃", TrayData->iTemperature2);
						else
				...
				if (TraySave.bMonitorDisk)
				{
					...
					if (VTray)
					{
						...
						if (hOHMA && TraySave.bMonitorTemperature)
						{
							...
							{
								DrawShadowText(mdc, TraySave.szDiskName, lstrlen(TraySave.szDiskName), &crc, DT_LEFT | DT_VCENTER | DT_SINGLELINE, bColor, bShadow);
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s", TrayData->iHddTemperature, TraySave.szTemperatureCPUUnit);
							}
							else if (TraySave.iMonitorSimple == 1)
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d℃", TrayData->iHddTemperature);
							else
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d", TrayData->iHddTemperature);
							DrawShadowText(mdc, sz, lstrlen(sz), &crc, DT_RIGHT | DT_VCENTER | DT_SINGLELINE, bColor, bShadow);
							OffsetRect(&crc, 0, wHeight);
							...
							{
								DrawShadowText(mdc, TraySave.szDiskName, lstrlen(TraySave.szDiskName), &crc, DT_LEFT | DT_VCENTER | DT_SINGLELINE, bColor, bShadow);
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%s", TrayData->disktime, TraySave.szUsageMEMUnit);
							}
							else if (TraySave.iMonitorSimple == 1)
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d%%", TrayData->disktime);
							else
								swprintf_s(sz, ARRAYSIZE(sz), L"%.2d", TrayData->disktime);
							DrawShadowText(mdc, sz, lstrlen(sz), &crc, DT_RIGHT | DT_VCENTER | DT_SINGLELINE, bColor, bShadow);
							OffsetRect(&crc, 0, wHeight);
				...
				if (TraySave.bMonitorTime)
				{
					...
					RECT crc = rc;
					TCHAR szWeek[7][2] = { L"日",L"一",L"二",L"三",L"四",L"五",L"六" };
					swprintf_s(sz, ARRAYSIZE(sz), L"%.2d/%.2d'%s", systm.wMonth, systm.wDay, szWeek[systm.wDayOfWeek]);
					int sLen = lstrlen(sz);
					if (VTray)
					{
						if (TraySave.bMonitorTraffic)
					...
					swprintf_s(sz, ARRAYSIZE(sz), L"%.2d:%.2d:%.2d", systm.wHour, systm.wMinute, systm.wSecond);
					sLen = lstrlen(sz);
					if (VTray)
						OffsetRect(&crc, 0, wHeight);
					else
...
*/
	...
	case WM_CLOSE:
	{
		...
		if (TraySave.bTrayStyle)
		{
			...
			while (hSecTray)
			{
				SetWindowCompositionAttribute(hSecTray, ACCENT_DISABLED, 0, (hWin11UI != NULL));
